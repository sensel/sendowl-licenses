...
function get_soft_auths(req,gets_bw){
  ...
  db.arturia.findOne({ order_id: '' }, function (err, onedoc) {
    var cart = [[],[]];
    if(onedoc!=null){
      cart[0] = [onedoc.serial,onedoc.unlock_code];
      update_db(req,onedoc._id,db.arturia);
      //satisfy order
      console.log('++ Arturia sn and unlock are '+cart[0][0]+' | '+cart[0][1]);
      ...
      cart[0] = 'contact support@sensel.com';
    }
    return cart
  });
}
